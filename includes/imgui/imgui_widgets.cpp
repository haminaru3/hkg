...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    it_anim->second.background = ImLerp(it_anim->second.background, IsItemActive() ? ImColor(25/255.f,25/255.f,25/255.f,1.f) : ImColor(19/255.f, 19/255.f, 19/255.f, 1.f), g.IO.DeltaTime * 6.f);
    it_anim->second.text = ImLerp(it_anim->second.text, IsItemActive() ? ImColor(77, 77, 77) : ImColor(255,255,255), g.IO.DeltaTime * 6.f);

    ImGui::GetWindowDrawList()->AddRectFilled(bb.Min, bb.Max, GetColorU32(it_anim->second.background), 6);
    ImGui::GetWindowDrawList()->AddRect(bb.Min, bb.Max, ImColor(1.f, 1.f, 1.f, 6 / 255.f), 6);
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    it_anim->second.text_anim = ImLerp(it_anim->second.text_anim, *v ? 1.0f : 0.6f, 0.12f * (1.0f - ImGui::GetIO().DeltaTime));

    RenderNavHighlight(total_bb, id);

    RenderFrame(frame_bb.Min, frame_bb.Max, ImColor(29, 29, 29), false, 9.0f);
    RenderFrame(frame_bb.Min, frame_bb.Max, ImColor(77 / 255.0f, 77 / 255.0f, 77 / 255.0f, it_anim->second.animation), false, 9.0f);

    window->DrawList->AddCircleFilled(ImVec2(frame_bb.Min.x + 8 + 14 * it_anim->second.animation, frame_bb.Min.y + 8), 5.3f, ImColor(1.0f, 1.0f, 1.0f), 360);

    if (label_size.x > 0.0f)
        window->DrawList->AddText(g.Font, g.FontSize, pos, ImColor(255/255.f,255/255.f,255/255.f, 0.7f), label);
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    it_anim->second.opacity = ImLerp(it_anim->second.opacity, IsItemActive() ? 1.0f : 0.5f, 0.15f * (1.0f - ImGui::GetIO().DeltaTime));
    ...
    //window->DrawList->AddRectFilled(frame_bb.Min, frame_bb.Max, ImColor(29/255.f, 29/255.f, 29/255.f, 1.f * s->Alpha), 5.0f);
    window->DrawList->AddRectFilled(frame_bb.Min, ImVec2(grab_bb.Min.x + 3, frame_bb.Max.y), ImColor(25/255.f, 25/255.f, 25/255.f, 1.f * s->Alpha), 5.0f);
    //window->DrawList->AddCircleFilled(ImVec2(grab_bb.Min.x - 1, grab_bb.Min.y + 1), 6.0f, ImColor(1.0f, 1.0f, 1.0f), 30);
    window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, ImColor(255/255.f, 255/255.f, 255/255.f, 0.04f * s->Alpha), 5.0f);
    window->DrawList->AddRect(frame_bb.Min, ImVec2(grab_bb.Min.x + 3, frame_bb.Max.y), ImColor(255 / 255.f, 255 / 255.f, 255 / 255.f, 0.04f * s->Alpha), 5.0f);

    PushStyleColor(ImGuiCol_Text, ImVec4(1.0f, 1.0f, 1.0f, 0.7f)); { //
        RenderText(total_bb.Min, label);
    } PopStyleColor();

    PushStyleColor(ImGuiCol_Text, ImVec4(1.0f, 1.0f, 1.0f, it_anim->second.opacity)); { //
        RenderTextClipped(total_bb.Min - ImVec2(6,0), total_bb.Max - ImVec2(6, 0), value_buf, value_buf_end, NULL, ImVec2(1.f, 0.f));
    } PopStyleColor();
...
bool ImGui::ColorEdit3(const char* label, std::array<float, 3> &col, ImGuiColorEditFlags flags) {
    return ColorEdit4(label, col.data(), flags | ImGuiColorEditFlags_NoAlpha);
}
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        PushStyleColor(ImGuiCol_PopupBg, GetColorU32(ImVec4(20/255.f, 20/255.f, 20/255.f, 1.f)));
        PushStyleColor(ImGuiCol_Border, GetColorU32(ImVec4(1.f, 1.f, 1.f, 0.07f)));
    ...
    if (label != label_display_end && !(flags & ImGuiColorEditFlags_NoLabel))
    {
        ...
        ImGui::GetWindowDrawList()->AddText(window->DC.CursorPos + ImVec2(2, 3), GetColorU32(ImVec4(1.f, 1.f, 1.f, 0.7f)), label);
...
// make ColorEdit4 compatible with std::array

bool ImGui::ColorEdit4(const char* label, std::array<float, 4> &col, ImGuiColorEditFlags flags)
{
	return ColorEdit4(label, col.data(), flags);
}
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    ImVec2 bar_pos = window->DC.CursorPos + ImVec2(0, 132);
    ...
    GetForegroundDrawList()->AddRectFilledMultiColorRounded(picker_pos, picker_pos + ImVec2(sv_picker_size, sv_picker_size - 80), GetColorU32(ImGuiCol_PopupBg), 0, 0, col_black, col_black, 4, ImDrawCornerFlags_All);

    GetForegroundDrawList()->AddRect(picker_pos, picker_pos + ImVec2(sv_picker_size, sv_picker_size - 80), ImColor(1.f, 1.f, 1.f, 0.1f), 4, ImDrawCornerFlags_All);
    ...
    // Render Hue Bar
    for (int i = 0; i < 6; ++i)
        GetForegroundDrawList()->AddRectFilledMultiColor(ImVec2(bar0_pos_x + i * (bars_width / 6), bar_pos.y + 8), ImVec2(bar0_pos_x + (i + 1) * (bars_width / 6), bar_pos.y + sv_bar_size - 4), col_hues[i], col_hues[i + 1], col_hues[i + 1], col_hues[i]);
    GetForegroundDrawList()->AddRectFilledMultiColorRounded(ImVec2(bar0_pos_x, bar_pos.y + 8), ImVec2(bar0_pos_x + bars_width, bar_pos.y + sv_bar_size - 4), GetColorU32(ImGuiCol_PopupBg), 0, 0, 0, 0, 30.f, ImDrawCornerFlags_All);
    ...
    GetForegroundDrawList()->AddCircleFilled(ImVec2(it_anim->second.hue_bar + bar_pos.x, bar_pos.y + 11), 6, ImColor(0, 0, 0, 255), 360.f);
    GetForegroundDrawList()->AddCircleFilled(ImVec2(it_anim->second.hue_bar + bar_pos.x, bar_pos.y + 11), 5, ImColor(255, 255, 255, 255), 360.f);
    ...
    GetForegroundDrawList()->AddCircleFilled(it_anim->second.circle_move + bar_pos, it_anim->second.circle, user_col32_striped_of_alpha, 360);
    GetForegroundDrawList()->AddCircleFilled(ImVec2(it_anim->second.hue_bar + bar_pos.x, bar_pos.y + 11), 7, ImColor(R / 2, G / 2, B / 2, 1.f), 32.f);
    GetForegroundDrawList()->AddCircleFilled(ImVec2(it_anim->second.hue_bar + bar_pos.x, bar_pos.y + 11), 5, user_col32_striped_of_alpha, 32.f);
    ...
    if (alpha_bar)
    {
        ...
        GetForegroundDrawList()->AddRectFilledMultiColorRounded(bar1_bb.Min + ImVec2(0, 8), bar1_bb.Max - ImVec2(0, 5), GetColorU32(ImGuiCol_PopupBg), 0, 0, 0, 0, 9, ImDrawCornerFlags_All);
        GetForegroundDrawList()->AddRect(bar1_bb.Min + ImVec2(0, 8), bar1_bb.Max - ImVec2(0, 5), ImColor(1.f, 1.f, 1.f, 0.07f), 9, ImDrawCornerFlags_All);
        ...
        GetForegroundDrawList()->AddCircleFilled(ImVec2(it_anim->second.alpha_bar + bar_pos.x, bar1_bb.Min.y + 10.0f), 7, ImColor(R / 2, G / 2, B / 2, 1.f), 32);
        GetForegroundDrawList()->AddCircleFilled(ImVec2(it_anim->second.alpha_bar + bar_pos.x, bar1_bb.Min.y + 10.0f), 5, IM_COL32(0, 0, 0, 255), 32);
        GetForegroundDrawList()->AddCircleFilled(ImVec2(it_anim->second.alpha_bar + bar_pos.x, bar1_bb.Min.y + 10.0f), 5, ImColor(R, G, B, col[3]), 32);
    }

    if (ImGui::Button("Copy", { 100, 28 })) {
        //colorpicker_buffer = ImColor(R, G, B, col[3]);
    }
    ImGui::SameLine();
    if (ImGui::Button("Paste", { 101, 28 })) {
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags, const ImVec2& size_arg)
{
    ...
    const float default_size = GetFrameHeight();
    const ImVec2 pos = window->DC.CursorPos;
    const float width = GetContentRegionMax().x - GetStyle().WindowPadding.x + 2;
    ...
    ImColor main_col = GetColorU32(col_rgb);
    ...
    RenderColorRectWithAlphaCheckerboard(window->DrawList, clickable.Min + ImVec2(3, 3), clickable.Max - ImVec2(3, 3), ImColor(0, 0, 0, 0), 3, ImVec2(0.f, 0.f), 2);
    window->DrawList->AddRectFilledMultiColor(clickable.Min + ImVec2(3, 3), clickable.Max - ImVec2(3, 3),
        main_col, main_col, halftone_col_2, halftone_col);

    window->DrawList->AddRect(clickable.Min, clickable.Max, ImColor(1.f, 1.f, 1.f, 30 / 255.f * s->Alpha), 3.0f);
    //ImGui::GetWindowDrawList()->AddRect(clickable.Min, clickable.Max, ImColor(1.f, 1.f, 1.f, 0.07f), 4);
    //GetWindowDrawList()->AddShadowRect(clickable.Min, clickable.Max + ImVec2(0, 0), GetColorU32(col_rgb), 20, ImVec2(0.f, 0.f), 4, 16);
...
bool ImGui::BeginMenuEx(const char* label, const char* icon, bool enabled)
{
    ...
    if (window->DC.LayoutType == ImGuiLayoutType_Horizontal)
    {
        ...
        pressed = false; //Selectable("", menu_is_open, selectable_flags, ImVec2(w, 0.0f));
    ...
    else
    {
        ...
        ImVec2 text_pos(window->DC.CursorPos.x + offsets->OffsetLabel, window->DC.CursorPos.y + window->DC.CurrLineTextBaseOffset);
        pressed = false;//Selectable("", menu_is_open, selectable_flags | ImGuiSelectableFlags_SpanAvailWidth, ImVec2(min_w, 0.0f));
        RenderText(text_pos, label);
        if (icon_w > 0.0f)
...
bool ImGui::MenuItemEx(const char* label, const char* icon, const char* shortcut, bool selected, bool enabled)
{
    ...
    if (window->DC.LayoutType == ImGuiLayoutType_Horizontal)
    {
        ...
        ImVec2 text_pos(window->DC.CursorPos.x + offsets->OffsetLabel, window->DC.CursorPos.y + window->DC.CurrLineTextBaseOffset);
        PushStyleVar(ImGuiStyleVar_ItemSpacing, ImVec2(style.ItemSpacing.x * 2.0f, style.ItemSpacing.y));
        pressed = false;//Selectable("", selected, selectable_flags, ImVec2(w, 0.0f));
        PopStyleVar();
        RenderText(text_pos, label);
        window->DC.CursorPos.x += IM_FLOOR(style.ItemSpacing.x * (-1.0f + 0.5f)); // -1 spacing to compensate the spacing added when Selectable() did a SameLine(). It would also work to call SameLine() ourselves after the PopStyleVar().
    }
    else
    {
        ...
        pressed = false;//Selectable("", false, selectable_flags | ImGuiSelectableFlags_SpanAvailWidth, ImVec2(min_w, 0.0f));

...
#define xorstr(str) ::jm::xor_string([]() { return str; }, std::integral_constant<std::size_t, sizeof(str) / sizeof(*str)>{}, std::make_index_sequence<::jm::detail::_buffer_size<sizeof(str)>()>{})
 // Compile time XOR string. So people can't view our strings via static analysis.
...
// Safe version of MessageBoxA, We are also hiding imports using this :)
#define SAFE_MESSAGESBOX(Content, Title) SAFE_CALL(MessageBoxExA)(0, E(Content), E(Title), 0, LANG_NEUTRAL)
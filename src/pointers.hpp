...
namespace pointers {
	...
	std::uint8_t* findPattern2(const char* module_name, const char* signature) {
		...
		logs::add(u_crypt("failed find pattern again in ") + string(module_name));
	...
	uint8_t* findPattern(const std::string_view module, const std::string_view signature) {
		...
		if (found_bytes == nullptr) {
			logs::add(u_crypt("failed find pattern, fallback ") + string(module.data()));
	...
	bool findAllPatterns() {
		...
		Game.ReplayInterface = *(CReplayInterface**)fix_mov(findPattern(game_executable, u_crypt("48 8D 0D ? ? ? ? 48 8B D7 E8 ? ? ? ? 48 8D 0D ? ? ? ? 8A D8 E8 ? ? ? ? 84 DB 75 13 48 8D 0D")));
		if (!IsValidPtr(Game.ReplayInterface)) {
			add_log("cant find ReplayInterface");
			return false;
		}

		Game.World = *(CWorld**)fix_mov(findPattern(game_executable, u_crypt("48 8B 05 ? ? ? ? 48 8B 58 08 48 85 DB 74 32")));
		...
		Game.viewPort = *(CViewPort**)fix_mov(findPattern(game_executable, u_crypt("48 8B 15 ? ? ? ? 48 8D 2D ? ? ? ? 48 8B CD")));


		Game.CamAddr = (uintptr_t)fix_mov(findPattern(game_executable, u_crypt("48 8B 05 ? ? ? ? 48 8B 98 ? ? ? ? EB")));
		...
		//GetBone = reinterpret_cast<GetBone_t>(findPattern(game_executable, u_crypt("48 89 5C 24 ? 48 89 6C 24 ? 48 89 74 24 ? 57 48 83 EC 60 48 8B 01 41 8B E8 48 8B F2 48 8B F9 33 DB")));
		GetBoneFunc = reinterpret_cast<GetBoneFromMask2>(findPattern(game_executable, u_crypt("48 89 5C 24 ? 48 89 6C 24 ? 48 89 74 24 ? 57 48 83 EC 60 48 8B 01 41 8B E8 48 8B F2 48 8B F9 33 DB")));
		if (!IsValidPtr(GetBoneFunc)) {
			add_log("cant find bone1");
			return false;
		}

		DWORD64 FrameCount_pattern = (DWORD64)findPattern(game_executable, u_crypt("8B 15 ? ? ? ? 41 FF CF"));
		...
		Is_Dlc_Present = reinterpret_cast<Is_Dlc_Present_t>(sub(findPattern(game_executable, u_crypt("57 48 83 EC 20 40 8A FA 8B 15 ? ? ? ? 48 8B F1")), u_crypt(0xA)));
		...
		_START_SHAPE_TEST_CAPSULE = (tSTART_SHAPE_TEST_CAPSULE)(findPattern(game_executable, u_crypt("41 8B CF C7 85 ? ? ? ? ? ? ? ? E8 ? ? ? ? B2 01")) - u_crypt(0xBE));
		...
		_GET_RAYCAST_RESULT = (t_GET_RAYCAST_RESULT)(findPattern(game_executable, u_crypt("48 89 5C 24 ? 48 89 74 24 ? 48 89 7C 24 ? 55 41 56 41 57 48 8B EC 48 83 EC 60 33 DB")));
		...
		pointer_to_handle = reinterpret_cast<pointer_to_handle_t>(findPattern(game_executable, u_crypt("48 89 5c 24 ? 48 89 74 24 ? 57 48 83 ec 20 8b 15 ? ? ? ? 48 8b f9 48 83 c1 10 33 db")));
		...
		give_weapon_delayed = reinterpret_cast<give_weapon_delayed_t>(findPattern(game_executable, u_crypt("48 89 5c 24 ? 48 89 6c 24 ? 48 89 74 24 ? 57 48 83 ec ? 41 8a e9 41 8b f0 8b fa e8 ? ? ? ? 48 8b d8 48 85 c0 74")));
		...
		weapon_patch_bytes_ptr = reinterpret_cast<uintptr_t>(findPattern(game_executable, u_crypt("0F 29 4F ?? 83 8F ?? ?? ?? ?? ?? 48 8B 4F")));
		if (!IsValidPtr(weapon_patch_bytes_ptr)) {
			add_log("cant find patch bytes ptr");
			return false;
		}

		gta_bullet_shot = reinterpret_cast<bullet_t>(findPattern(game_executable, u_crypt("8B 05 ?? ?? ?? ?? F3 41 0F 10 10")));
		if (!IsValidPtr(gta_bullet_shot)) {
			add_log("cant find bullet ptr");
			return false;
		}

		gta_bullet_instance = reinterpret_cast<sBulletInstance>(findPattern(game_executable, u_crypt("48 89 5C 24 08 57 48 83 EC 30 41 8B 00 F3")));
		if (!IsValidPtr(gta_bullet_shot)) {
			add_log("cant find bullet instance ptr");
			return false;
		}

		//magic bullet??
		//E8 ? ? ? ? 8B 75 D4
		/*gta_event_gun_shot = fix_call<gta_event_gun_shot_t>(findPattern(game_executable, u_crypt("48 89 5C 24 ? 57 48 83 EC 30 41 8B 00")));
		...
		/*HandleBullet = (HandleBullet_t)findPattern(game_executable, "48 89 5C 24 ? 57 48 83 EC 30 41 8B 00");
		if (!IsValidPtr(HandleBullet)) {
			add_log("cant find HandleBullet");
		}*/
		...
		gta_script_thread_tick = reinterpret_cast<script_thread_tick_t>(sub(findPattern(game_executable, u_crypt("48 83 EC 20 80 B9 4E 01 00 00 00")), u_crypt(0xB))); // for 1.61
		...
		gta_get_native_handler = reinterpret_cast<get_native_handler_t>(as_relative(findPattern(game_executable, u_crypt("48 8D 0D ? ? ? ? 48 8B 14 FA E8 ? ? ? ? 48 85 C0 75 0A")), 12));
		...
		// sig from https://github.com/Yimura/GTA5Base/blob/42454e20c67de80a01140fa85cbe2aadfad12728/src/Pointers.cpp#L41
		gta_native_registration_table = reinterpret_cast<uintptr_t*>(as_relative(findPattern(game_executable, u_crypt("48 8D 0D ? ? ? ? 48 8B 14 FA E8 ? ? ? ? 48 85 C0 75 0A")), 3));
		...
		gta_fix_context_vector = reinterpret_cast<fix_context_vector_t>(findPattern(game_executable, u_crypt("83 79 18 00 48 8B D1 74 4A FF 4A 18 48 63 4A 18 48 8D 41 04 48 8B 4C CA")));
		...
		if (GetModuleHandleA("multiplayerL.dll") || GetModuleHandleA("multiplayer.dll")) {
			...
			//}
			//
			//ragemp_fetch_handler = reinterpret_cast<ragemp_fetch_handler_t>(as_relative(findPattern(ragemp_exec, u_crypt("E8 ? ? ? ? 48 89 05 ? ? ? ? 48 8D 0D ? ? ? ? E8 ? ? ? ? E9 ? ? ? ?")), 1));

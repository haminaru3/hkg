...
ostream& operator<<(ostream& os, const PVector3& vector)
{
...
HandleBullet_t OriginalHandleBullet = nullptr;

typedef __int64(__fastcall* bullet_t)(__int64 CWeapon, Vector3 vend, Vector3 vend2);
bullet_t gta_bullet_shot = nullptr;
bullet_t original_bullet_shot = nullptr;
...
sBulletInstance gta_bullet_instance = nullptr;
sBulletInstance original_bullet_instance = nullptr;

typedef char(__fastcall* gta_fire_type)(__int64 a1, uint64_t *a2);
gta_fire_type gta_weapon_fire = nullptr;
gta_fire_type original_weapon_fire = nullptr;

uintptr_t weapon_patch_bytes_ptr;
...
public:
	...
	uint32_t GetHash() {
		return (*(uint32_t*)((DWORD64)this + 0x18));
	}

	char(&GetName())[20] {
		return *reinterpret_cast<char(*)[20]>((DWORD64)this + 0x298);
	}

	char(&GetManufacturer())[20] {
		return *reinterpret_cast<char(*)[20]>((DWORD64)this + 0x2A4);
	}
...
public:
	...
	uintptr_t getDriver() {
		return (uintptr_t)((DWORD64)this + 0x0CA8); // this shit is foul cuz CPed depends on CVehicle and i cant use CPed before declaration. co-dependance issue basically.
	}
...
public:
	...
	void set_muzzle_start(Vector3 pos) {

		*(Vector3*)((DWORD64) this + 0x20) = pos;

	}

	Vector3 get_muzzle_start() {

		return (*(Vector3*)((DWORD64)this + 0x20));
...
public:
	...
	float getReloadSpeed() {
		return *(float*)((DWORD64)this + 0x134);
	}
	...
	uint64_t GetWeaponHash() {
		return *(uint64_t*)((DWORD64)this + 0x10); // return (*reinterpret_cast<char**>((DWORD64)this + 0x5F0));
	}
...
//ok this is the shit i need to do to print char as hex number lol
...
class CWeaponPatcher {
private:
	unsigned char originalbytes[4];
	unsigned char patchbytes[4] = { 0x90, 0x90, 0x90, 0x90 };

public:
	void init() {
		originalbytes[0] = (*(BYTE*)((DWORD64)weapon_patch_bytes_ptr));
		originalbytes[1] = (*(BYTE*)((DWORD64)weapon_patch_bytes_ptr + 0x1));
		originalbytes[2] = (*(BYTE*)((DWORD64)weapon_patch_bytes_ptr + 0x2));
		originalbytes[3] = (*(BYTE*)((DWORD64)weapon_patch_bytes_ptr + 0x3));	

		//std::cout << "saved patch: " << hex(originalbytes[0]) << " " << hex(originalbytes[1]) << " " << hex(originalbytes[2]) << " " << hex(originalbytes[3]) << std::endl;
	}
	
	void patch() {
		(*(BYTE*)((DWORD64)weapon_patch_bytes_ptr)) = patchbytes[0];
		(*(BYTE*)((DWORD64)weapon_patch_bytes_ptr + 0x1)) = patchbytes[1];
		(*(BYTE*)((DWORD64)weapon_patch_bytes_ptr + 0x2)) = patchbytes[2];
		(*(BYTE*)((DWORD64)weapon_patch_bytes_ptr + 0x3)) = patchbytes[3];

		//std::cout << "patched bytes: " << hex((*(unsigned char*)((DWORD64)weapon_patch_bytes_ptr))) << " " << hex((*(unsigned char*)((DWORD64)weapon_patch_bytes_ptr + 0x1))) << " " << hex((*(unsigned char*)((DWORD64)weapon_patch_bytes_ptr + 0x2))) << " " << hex((*(unsigned char*)((DWORD64)weapon_patch_bytes_ptr + 0x3))) << std::endl;
	}

	void unpatch() {
		(*(BYTE*)((DWORD64)weapon_patch_bytes_ptr)) = originalbytes[0];
		(*(BYTE*)((DWORD64)weapon_patch_bytes_ptr + 0x1)) = originalbytes[1];
		(*(BYTE*)((DWORD64)weapon_patch_bytes_ptr + 0x2)) = originalbytes[2];
		(*(BYTE*)((DWORD64)weapon_patch_bytes_ptr + 0x3)) = originalbytes[3];

		//std::cout << "unpatched bytes: " << hex((*(BYTE*)((DWORD64)weapon_patch_bytes_ptr))) << " " << hex((*(BYTE*)((DWORD64)weapon_patch_bytes_ptr + 0x1))) << " " << hex((*(BYTE*)((DWORD64)weapon_patch_bytes_ptr + 0x2))) << " " << hex((*(unsigned char*)((DWORD64)weapon_patch_bytes_ptr + 0x3))) << std::endl;

	}

	void getStatus(unsigned char& output1, unsigned char& output2, unsigned char& output3, unsigned char& output4) {
		output1 = (*(BYTE*)((DWORD64)weapon_patch_bytes_ptr));
		output2 = (*(BYTE*)((DWORD64)weapon_patch_bytes_ptr + 0x1));
		output3 = (*(BYTE*)((DWORD64)weapon_patch_bytes_ptr + 0x2));
		output4 = (*(BYTE*)((DWORD64)weapon_patch_bytes_ptr + 0x3));
	}

};

class CAbstractWeaponObject {
public:
	CWeapon* get_cweapon() {
		return *(CWeapon**)((uintptr_t)this + 0x320);
	}
};
...
public:
	...
	CAbstractWeaponObject* weaponobj() {
		return *(CAbstractWeaponObject**)((uintptr_t)this + 0x78);
	}
}; //Size: 0x01E8
...
struct PlayerBones {
	...
	// пальци
	Vector3 LEFT_FINGER0;
	Vector3 LEFT_FINGER1;
	Vector3 LEFT_FINGER2;
	Vector3 LEFT_FINGER10;
	Vector3 LEFT_FINGER11;
	Vector3 LEFT_FINGER12;
	Vector3 LEFT_FINGER20;
	Vector3 LEFT_FINGER21;
	Vector3 LEFT_FINGER22;
	Vector3 LEFT_FINGER30;
	Vector3 LEFT_FINGER31;
	Vector3 LEFT_FINGER32;
	Vector3 LEFT_FINGER40;
	Vector3 LEFT_FINGER41;
	Vector3 LEFT_FINGER42;

	Vector3 RIGHT_FINGER0;
	Vector3 RIGHT_FINGER1;
	Vector3 RIGHT_FINGER2;
	Vector3 RIGHT_FINGER10;
	Vector3 RIGHT_FINGER11;
	Vector3 RIGHT_FINGER12;
	Vector3 RIGHT_FINGER20;
	Vector3 RIGHT_FINGER21;
	Vector3 RIGHT_FINGER22;
	Vector3 RIGHT_FINGER30;
	Vector3 RIGHT_FINGER31;
	Vector3 RIGHT_FINGER32;
	Vector3 RIGHT_FINGER40;
	Vector3 RIGHT_FINGER41;
	Vector3 RIGHT_FINGER42;
};
...
public:
	...
	uint32_t weapon_hash() {
		return (uint32_t)((uintptr_t)this + 0x10B8 + 0x18);
	}

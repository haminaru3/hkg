...
#pragma warning(pop)
	...
	namespace invoker {
		...
		inline type invoke(uintptr_t hash, args_t&& ...args) {
			if (tickbase::get_since_last_tick() > 20) {
				logs::add("LATE NATIVE!!!!!!!!!!!!!!!!");
			}
...
#ifdef _DEBUG
			if (g_config->misc.debug.native_logger && (g_config->misc.debug.nlog_console || g_config->misc.debug.nlog_file)) {
				std::ostringstream output;
				output << std::format("invoked 0x{:x}(", hash);

				if constexpr (sizeof...(args) > 0) {
					std::ostringstream oss;
					// Capture oss by reference
					auto print_args = [&oss](const auto& arg) {
						oss << arg_to_string(arg) << ", ";
						};

					(print_args(args), ...);
					std::string ossstring = oss.str();
					if (!ossstring.empty()) {
						ossstring.pop_back(); // Remove last comma
						ossstring.pop_back(); // Remove last space
					}
					output << ossstring;
				}

				output << ") | TICK: " << Game.tick_count;
				if (g_config->misc.debug.nlog_console) std::cout << output.str() << std::endl;
				if (g_config->misc.debug.nlog_file) logs::add(output.str());
			}
...
#pragma warning(disable:4505)
	...
	namespace weapon {
		...
		static float get_weapon_damage(type::hash weaponhash) {
			return invoker::invoke<float>(0x3133b907d8b32053, weaponhash, nullptr);
		}
